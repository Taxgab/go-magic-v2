...
function PagoModal({ pago, alumnos, onClose, onSave }: { pago: Pago | null, alumnos: Alumno[], onClose: () => void, onSave: () => void }) {
  ...
  const handleChange = (field: keyof PagoInsert, value: string | number | MetodoPago | PagoEstado) => {
      ...
      <div className="bg-white rounded-xl max-w-lg w-full">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            {formErrors.alumno_id && <p className="mt-1 text-sm text-red-600">{formErrors.alumno_id}</p>}
          </div>
          ...
          <div>
            ...
            />
            {formErrors.concepto && <p className="mt-1 text-sm text-red-600">{formErrors.concepto}</p>}
          </div>
